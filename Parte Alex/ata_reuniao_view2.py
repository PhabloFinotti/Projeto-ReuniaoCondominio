...
class Ata_ReuniaoView:
    ...
    def __init__(self,win):
        ...
        #Tabela Presença
        self.presencaLabel = tk.Label(win, text="Presente/Ausente:")
        self.presencaEdit = tk.Entry(win, width=15,bd=3)

        self.apartamentoLabel = tk.Label(win, text="N° AP:")
        self.apartamentoEdit = ttk.Combobox(win, width=2, values=[])
        self.apartamentoEdit.bind("<<ComboboxSelected>>", self._on_combo_changed_ap)

        self.reuniaoLabel = tk.Label(win, text="Tema:")
        self.reuniaoCombo = ttk.Combobox(win, width=10, values=[])
        self.reuniaoCombo.bind("<<ComboboxSelected>>", self._on_combo_changed_reuniao)

        self.popular_combo_apartamento()
        self.popular_combo_reuniao()
        ...
        #self.btnBuscarPresenca = tk.Button(win,
        #        text='Buscar', width=7,command=self._on_buscar_clicked)
        self.btnCriar = tk.Button(win,
                text="CriarTables", width= 7, command=self._criar_tables_clicked)
        ...
        #self.ata_reuniaoList.heading(3, text='ID Reuni')


        self.ata_reuniaoList.column(1, minwidth=0, width=0)
        #self.ata_reuniaoList.column(2, minwidth=0, width=165)
        self.ata_reuniaoList.column(2, minwidth=0, width=195)
        #self.ata_reuniaoList.column(3, minwidth=0, width=0)
        ...
        #Tabela Presença
        self.presencaLabel.place(x=418,y=10)
        self.presencaEdit.place(x=420,y=30)

        self.apartamentoLabel.place(x=277,y=10)
        self.apartamentoEdit.place(x=280,y=30)

        self.reuniaoLabel.place(x=328,y=10)
        self.reuniaoCombo.place(x=330,y=30)

        #TreeView Presença
        self.reuniaoList = ttk.Treeview(win, columns=(1,2,3,4,5), show='headings')
        self.verscrlbar = ttk.Scrollbar(win,orient="vertical", command=self.reuniaoList.yview)
        self.verscrlbar.pack(side = 'right', fill='x')
        self.reuniaoList.configure(yscrollcommand = self.verscrlbar.set)

        self.reuniaoList.heading(1, text='ID')
        self.reuniaoList.heading(2, text='N°AP')
        self.reuniaoList.heading(3, text='Nome Morador')
        self.reuniaoList.heading(4, text='Tema')
        self.reuniaoList.heading(5, text='Presente/Ausente')

        self.reuniaoList.column(1, minwidth=0, width=0)
        self.reuniaoList.column(2, minwidth=0, width=50) 
        self.reuniaoList.column(3, minwidth=0, width=115)
        self.reuniaoList.column(4, minwidth=0, width=100)
        self.reuniaoList.column(5, minwidth=0, width=135)
        self.reuniaoList.pack()

        self.reuniaoList.bind("<<TreeviewSelect>>", self._on_mostrar_clicked_Presenca)

        #Posicionar List e scroll
        self.reuniaoList.place(x=280,y=55, height= 185)
        ...
        #Presença
        self.btnConfirma.place(x=550,y=28)
        self.btnAlterarP.place(x=625,y=28)
        self.btnExcluirP.place(x=625,y=240)
        self.btnCriar.place(x=220,y=6)
        #self.btnBuscarPresenca.place(x=460, y=30)
    ...
    def popular_combo_apartamento(self):
        ...
        if (len(self.apartamentoResult)> 0):
            self.apartamentoSelected = self.apartamentoResult[0][1] #1 e do campo que deseja ser salvo

            for registro in self.apartamentoResult:
                self.apartamentoEdit['values']=(*self.apartamentoEdit['values'],registro[1])       
            self.apartamentoEdit.current(0)

    def popular_combo_reuniao(self):
        reuniao = Ata_Reuniao() 

        self.reuniaoResult = reuniao.consultar_reuniao() 
        if (len(self.reuniaoResult)> 0):
            self.reuniaoSelected = self.reuniaoResult[0][0] #antes 0 para salvar Id

            for registro in self.reuniaoResult:
                self.reuniaoCombo['values']=(*self.reuniaoCombo['values'],registro[2])       
            self.reuniaoCombo.current(0)

    
    def carregar_dados_iniciais(self):
        if (self.apartamentoSelected != None):
            resultado = self.ata_reuniaoCRUD.consultar_por_apartamento(self.apartamentoSelected) 
            self.reuniaoList.delete(*self.reuniaoList.get_children())

            count = 0 
            for registro in resultado:
                self.reuniaoList.insert('', 'end',iid=count,values=(str(registro[0]),str(registro[1]),(registro[1]),(registro[2]),registro[3]))
                count = count + 1

    #def _on_buscar_clicked(self):
    #    #reuniaobusca = self.presencaEdit.get()

    #    ata_reuniao = Ata_Reuniao()
    #    self.resultado_presenca = ata_reuniao.consultar_tabela_presenca(reuniaobusca)
      
    #    count = 0
    #    for registro in self.resultado_presenca:
    #        self.reuniaoList.insert('','end',iid=(count),values=(str(registro[0]),str(registro[1]),(registro[4])))
    #        count = count + 1
    ...
    def _on_mostrar_clicked_Presenca(self, event):
        ...
        self.presencaEdit.insert(0,presenca)


    def _on_confirma_clicked(self):
        presenca = self.presencaEdit.get()
        #Ap = self.apartamentoEdit.get()

        if self.ata_reuniaoCRUD.confirma(presenca,self.apartamentoSelected,self.reuniaoSelected) == True:
    ...
    def _on_alterar_clicked(self):
        ...
        if (len(linhaSelecionada) != 0 ): 
            id_presenca = self.reuniaoList.item(linhaSelecionada[0])["values"][0]  
            presenca = self.presencaEdit.get() 
            reuniao_id = self.reuniaoSelected
    ...
    def _on_combo_changed_ap(self, event):
        index = self.apartamentoEdit.current()
        self.apartamentoSelected = self.apartamentoResult[index][0]
        self.carregar_dados_iniciais()

    def _on_combo_changed_reuniao(self, event):
        index = self.reuniaoCombo.current()
        self.reuniaoSelected = self.reuniaoResult[index][0]
        #self.carregar_dados_iniciais_treeView()


    def _criar_tables_clicked(self):
